function varargout = qu11(varargin)
% QU11 MATLAB code for qu11.fig
%      QU11, by itself, creates a new QU11 or raises the existing
%      singleton*.
%
%      H = QU11 returns the handle to a new QU11 or the handle to
%      the existing singleton*.
%
%      QU11('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QU11.M with the given input arguments.
%
%      QU11('Property','Value',...) creates a new QU11 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before qu11_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to qu11_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help qu11

% Last Modified by GUIDE v2.5 14-Jan-2016 23:28:17

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @qu11_OpeningFcn, ...
                   'gui_OutputFcn',  @qu11_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before qu11 is made visible.
function qu11_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to qu11 (see VARARGIN)

% Choose default command line output for qu11
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
set(handles.axes1,'visible','off')
ques_11=fileread('questions_11.txt');
ans_11=fileread('answer_11.txt');
l=1:1:10;
n=length(l);
number=(randi(n));

ques=strsplit(ques_11,'!');
q=(ques{number});
ans=strsplit(ans_11,'!');
mc=(ans{number});
mcq=strsplit(mc,',');
...
h = imread('moneybar11.png');
axes(handles.axes2)
imshow(h);
% UIWAIT makes qu11 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = qu11_OutputFcn(hObject, eventdata, handles) 
...
set(handles.text2,'string','You have won £64000')
drawnow()
pause(2)
close
run qu12.m
