function varargout = qu6(varargin)
% QU6 MATLAB code for qu6.fig
%      QU6, by itself, creates a new QU6 or raises the existing
%      singleton*.
%
%      H = QU6 returns the handle to a new QU6 or the handle to
%      the existing singleton*.
%
%      QU6('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QU6.M with the given input arguments.
%
%      QU6('Property','Value',...) creates a new QU6 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before qu6_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to qu6_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help qu6

% Last Modified by GUIDE v2.5 14-Jan-2016 23:22:22

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @qu6_OpeningFcn, ...
                   'gui_OutputFcn',  @qu6_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before qu6 is made visible.
function qu6_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to qu6 (see VARARGIN)

% Choose default command line output for qu6
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
set(handles.axes1,'visible','off')
ques_6=fileread('questions_6.txt');
ans_6=fileread('answer_6.txt');
l=1:1:10;
n=length(l);
number=(randi(n));

ques=strsplit(ques_6,'!');
q=(ques{number});
ans=strsplit(ans_6,'!');
mc=(ans{number});
mcq=strsplit(mc,',');
...
h = imread('moneybar6.png');
axes(handles.axes2)
imshow(h);


% UIWAIT makes qu6 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = qu6_OutputFcn(hObject, eventdata, handles) 
...
set(handles.text2,'string','You have won £2000!');
drawnow()
pause(2)
close
run qu7.m
