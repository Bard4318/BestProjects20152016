function varargout = qu3(varargin)
% QU3 MATLAB code for qu3.fig
%      QU3, by itself, creates a new QU3 or raises the existing
%      singleton*.
%
%      H = QU3 returns the handle to a new QU3 or the handle to
%      the existing singleton*.
%
%      QU3('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QU3.M with the given input arguments.
%
%      QU3('Property','Value',...) creates a new QU3 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before qu3_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to qu3_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help qu3

% Last Modified by GUIDE v2.5 14-Jan-2016 23:18:55

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @qu3_OpeningFcn, ...
                   'gui_OutputFcn',  @qu3_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before qu3 is made visible.
function qu3_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to qu3 (see VARARGIN)

% Choose default command line output for qu3
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
set(handles.axes1,'visible','off')
ques_3=fileread('questions_3.txt');
ans_3=fileread('answer_3.txt');
l=1:1:10;
n=length(l);
number=(randi(n));

ques=strsplit(ques_3,'!');
q=(ques{number});
ans=strsplit(ans_3,'!');
mc=(ans{number});
mcq=strsplit(mc,',');
...
% UIWAIT makes qu3 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = qu3_OutputFcn(hObject, eventdata, handles) 
...
set(handles.text2,'string','You have won £300!');
drawnow()
pause(2)
close
run qu4.m
