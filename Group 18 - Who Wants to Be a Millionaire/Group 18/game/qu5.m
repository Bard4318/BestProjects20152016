function varargout = qu5(varargin)
% QU5 MATLAB code for qu5.fig
%      QU5, by itself, creates a new QU5 or raises the existing
%      singleton*.
%
%      H = QU5 returns the handle to a new QU5 or the handle to
%      the existing singleton*.
%
%      QU5('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QU5.M with the given input arguments.
%
%      QU5('Property','Value',...) creates a new QU5 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before qu5_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to qu5_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help qu5

% Last Modified by GUIDE v2.5 14-Jan-2016 23:20:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @qu5_OpeningFcn, ...
                   'gui_OutputFcn',  @qu5_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before qu5 is made visible.
function qu5_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to qu5 (see VARARGIN)

% Choose default command line output for qu5
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
set(handles.axes1,'visible','off')
ques_5=fileread('questions_5.txt');
ans_3=fileread('answer_5.txt');
l=1:1:10;
n=length(l);
number=(randi(n));

ques=strsplit(ques_5,'!');
q=(ques{number});
ans=strsplit(ans_3,'!');
mc=(ans{number});
mcq=strsplit(mc,',');
...
h = imread('moneybar5.png');
axes(handles.axes2)
imshow(h);
% UIWAIT makes qu5 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = qu5_OutputFcn(hObject, eventdata, handles) 
...
set(handles.text2,'string','You have won £1000');
drawnow()
pause(2)
close
run qu6.m
