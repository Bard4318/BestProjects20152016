function varargout = qu12(varargin)
% QU12 MATLAB code for qu12.fig
%      QU12, by itself, creates a new QU12 or raises the existing
%      singleton*.
%
%      H = QU12 returns the handle to a new QU12 or the handle to
%      the existing singleton*.
%
%      QU12('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QU12.M with the given input arguments.
%
%      QU12('Property','Value',...) creates a new QU12 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before qu12_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to qu12_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help qu12

% Last Modified by GUIDE v2.5 14-Jan-2016 23:29:03

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @qu12_OpeningFcn, ...
                   'gui_OutputFcn',  @qu12_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before qu12 is made visible.
function qu12_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to qu12 (see VARARGIN)

% Choose default command line output for qu12
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
set(handles.axes1,'visible','off')
ques_12=fileread('questions_12.txt');
ans_12=fileread('answer_12.txt');
l=1:1:10;
n=length(l);
number=(randi(n));

ques=strsplit(ques_12,'!');
q=(ques{number});
ans=strsplit(ans_12,'!');
mc=(ans{number});
mcq=strsplit(mc,',');
...
h = imread('moneybar12.png');
axes(handles.axes2)
imshow(h);
% UIWAIT makes qu12 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = qu12_OutputFcn(hObject, eventdata, handles) 
...
set(handles.text2,'string','You have won £125000')
drawnow()
pause(2)
close
run qu13.m
