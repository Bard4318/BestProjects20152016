function varargout = qu15(varargin)
% QU15 MATLAB code for qu15.fig
%      QU15, by itself, creates a new QU15 or raises the existing
%      singleton*.
%
%      H = QU15 returns the handle to a new QU15 or the handle to
%      the existing singleton*.
%
%      QU15('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in QU15.M with the given input arguments.
%
%      QU15('Property','Value',...) creates a new QU15 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before qu15_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to qu15_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help qu15

% Last Modified by GUIDE v2.5 14-Jan-2016 23:31:50

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @qu15_OpeningFcn, ...
                   'gui_OutputFcn',  @qu15_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before qu15 is made visible.
function qu15_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to qu15 (see VARARGIN)

% Choose default command line output for qu15
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);
set(handles.axes1,'visible','off')
ques_15=fileread('questions_15.txt');
ans_15=fileread('answer_15.txt');
l=1:1:10;
n=length(l);
number=(randi(n));

ques=strsplit(ques_15,'!');
q=(ques{number});
ans=strsplit(ans_15,'!');
mc=(ans{number});
mcq=strsplit(mc,',');
...
h = imread('moneybar15.png');
axes(handles.axes2)
imshow(h);

% UIWAIT makes qu15 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = qu15_OutputFcn(hObject, eventdata, handles) 
...
set(handles.text2,'string','You are now a millionaire!!')
drawnow()
pause(2)
close
run Finish.m
